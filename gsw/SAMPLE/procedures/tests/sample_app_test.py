...
def run_sample_app_test():
    ...
    ##
    for n in range(SAMPLE_TEST_LOOP_COUNT):
        sample_cmd("SAMPLE_DEBUG SAMPLE_NOOP_CC")
    ...
    for n in range(SAMPLE_TEST_LOOP_COUNT):
        sample_cmd("SAMPLE_DEBUG SAMPLE_NOOP_CC")
        cmd("SAMPLE_DEBUG SAMPLE_RST_COUNTERS_CC") # Note standard `cmd` as we can't reset counters and then confirm increment
        get_sample_hk()
        check("SAMPLE_DEBUG SAMPLE_HK_TLM CMD_COUNT == 0")
        check("SAMPLE_DEBUG SAMPLE_HK_TLM CMD_ERR_COUNT == 0")
    ...
    for n in range(SAMPLE_TEST_LOOP_COUNT):
        ...
        cmd(f"SAMPLE_DEBUG SAMPLE_NOOP_CC with CCSDS_LENGTH {n+2}") # Note +2 due to CCSDS already being +1
        get_sample_hk()
    ...
    for n in range(6, 6 + SAMPLE_TEST_LOOP_COUNT): 
        cmd_cnt = tlm("SAMPLE_DEBUG SAMPLE_HK_TLM CMD_COUNT")
        cmd_err_cnt = tlm("SAMPLE_DEBUG SAMPLE_HK_TLM CMD_ERR_COUNT")
        cmd(f"SAMPLE_DEBUG SAMPLE_NOOP_CC with CCSDS_FC {n+1}")
        get_sample_hk()
        check(f"SAMPLE_DEBUG SAMPLE_HK_TLM CMD_COUNT == {cmd_cnt}")
        check(f"SAMPLE_DEBUG SAMPLE_HK_TLM CMD_ERR_COUNT == {cmd_err_cnt+1}")