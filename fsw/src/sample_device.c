...
int32 SAMPLE_RawIO(void)
{
    ...
    if (OS_MutSemTake(SAMPLE_AppData.DeviceMutex) == OS_SUCCESS)
    {
        ...
        if (bytes_available > 0)
        {
            ...
            {
                CFE_EVS_SendEvent(SAMPLE_UART_READ_ERR_EID, CFE_EVS_ERROR, "SAMPLE: RawIO Uart read error, expected %u and returned %u", bytes_available, bytes);
...
int32 SAMPLE_CommandDevice(uint8* cmd)
{
    ...
    if (OS_MutSemTake(SAMPLE_AppData.DeviceMutex) == OS_SUCCESS)
    {
        ...
        {
            CFE_EVS_SendEvent(SAMPLE_UART_WRITE_ERR_EID, CFE_EVS_ERROR, "SAMPLE: Command uart write error, expected %lu and returned %u", SAMPLE_DEVICE_CMD_LNGTH, bytes);
            status = OS_ERROR;
            OS_MutSemGive(SAMPLE_AppData.DeviceMutex);
            return status;
        }
