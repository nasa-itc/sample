    ...
    while (SAMPLE_GetRunStatus() == CFE_ES_APP_RUN)
    {
        ...
        if (OS_MutSemTake(SAMPLE_AppData.DeviceMutex) == OS_SUCCESS)
        {
            ...
            if (bytes_available >= SAMPLE_DEVICE_STREAM_LNGTH - SAMPLE_DEVICE_HDR_LEN)
            {
                ...
                {
                    CFE_EVS_SendEvent(SAMPLE_UART_READ_ERR_EID, CFE_EVS_ERROR, "SAMPLE: Streaming Uart read error, expected %lu and returned %u", SAMPLE_DEVICE_STREAM_LNGTH - SAMPLE_DEVICE_HDR_LEN, bytes);
                    /* Drop data */
                }
                else
                {
