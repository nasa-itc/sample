...
*/
int32_t SAMPLE_ReadData(uart_info_t *device, uint8_t *read_data,
                        uint8_t data_length) {
  ...
  while ((bytes_available < data_length) &&
         (ms_timeout_counter < SAMPLE_CFG_MS_TIMEOUT)) {
    ms_timeout_counter++;
    OS_TaskDelay(1);
    bytes_available = uart_bytes_available(device);
  }
...
*/
int32_t SAMPLE_CommandDevice(uart_info_t *device, uint8_t cmd_code,
                             uint32_t payload) {
  int32_t status = OS_SUCCESS;
  int32_t bytes = 0;
  ...
  if (status == UART_SUCCESS) {
    ...
    if (bytes == SAMPLE_DEVICE_CMD_SIZE) {
      ...
      if (status == OS_SUCCESS) {
        ...
        bytes = 0;
        while ((bytes < (int32_t)SAMPLE_DEVICE_CMD_SIZE) &&
               (status == OS_SUCCESS)) {
...
#ifdef SAMPLE_CFG_DEBUG
      OS_printf(
          "SAMPLE_CommandDevice - uart_write_port returned %d, expected %d \n",
...
*/
int32_t SAMPLE_RequestData(uart_info_t *device,
