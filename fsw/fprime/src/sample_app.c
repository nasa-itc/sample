...
void SAMPLE_AppMain(void) {
  ...
  */
  while (CFE_ES_RunLoop(&SAMPLE_AppData.RunStatus) == true) {
    ...
    */
    status = CFE_SB_ReceiveBuffer((CFE_SB_Buffer_t **)&SAMPLE_AppData.MsgPtr,
    ...
    */
    if (status == CFE_SUCCESS) {
      ...
      CFE_EVS_SendEvent(SAMPLE_PIPE_ERR_EID, CFE_EVS_EventType_ERROR,
                        "SAMPLE: SB Pipe Read Error = %d", (int)status);
      SAMPLE_AppData.RunStatus = CFE_ES_RunStatus_APP_ERROR;
    }
  }
...
int32 SAMPLE_AppInit(void) {
  ...
  */
  ...
  if (status != CFE_SUCCESS) {
    CFE_ES_WriteToSysLog(
        "SAMPLE: Error registering for event services: 0x%08X\n",
        (unsigned int)status);
    return status;
  }
  ...
  */
  status = CFE_SB_CreatePipe(&SAMPLE_AppData.CmdPipe, SAMPLE_PIPE_DEPTH,
                             "SAMPLE_CMD_PIPE");
  if (status != CFE_SUCCESS) {
    CFE_EVS_SendEvent(SAMPLE_PIPE_ERR_EID, CFE_EVS_EventType_ERROR,
  ...
  */
  status = CFE_SB_Subscribe(CFE_SB_ValueToMsgId(SAMPLE_CMD_MID),
  ...
  */
  status = CFE_SB_Subscribe(CFE_SB_ValueToMsgId(SAMPLE_REQ_HK_MID),
                            SAMPLE_AppData.CmdPipe);
  if (status != CFE_SUCCESS) {
  ...
  */
  CFE_MSG_Init(CFE_MSG_PTR(SAMPLE_AppData.HkTelemetryPkt.TlmHeader),
               CFE_SB_ValueToMsgId(SAMPLE_HK_TLM_MID), SAMPLE_HK_TLM_LNGTH);
  ...
  */
  ...
  status = CFE_EVS_SendEvent(
      SAMPLE_STARTUP_INF_EID, CFE_EVS_EventType_INFORMATION,
...
void SAMPLE_ProcessCommandPacket(void) {
  ...
  /*
  ** Ground Commands with command codes fall under the SAMPLE_CMD_MID (Message
  *ID)
  */
  case SAMPLE_CMD_MID:
...
void SAMPLE_ProcessGroundCommand(void) {
  ...
  */
  case SAMPLE_NOOP_CC:
    ...
    */
    if (SAMPLE_VerifyCmdLength(SAMPLE_AppData.MsgPtr,
                               sizeof(SAMPLE_NoArgs_cmd_t)) == OS_SUCCESS) {
  ...
  */
  case SAMPLE_ENABLE_CC:
    if (SAMPLE_VerifyCmdLength(SAMPLE_AppData.MsgPtr,
                               sizeof(SAMPLE_NoArgs_cmd_t)) == OS_SUCCESS) {
      CFE_EVS_SendEvent(SAMPLE_CMD_ENABLE_INF_EID,
                        CFE_EVS_EventType_INFORMATION,
                        "SAMPLE: Enable command received");
      SAMPLE_Enable();
    }
    break;
  ...
  */
  case SAMPLE_DISABLE_CC:
    if (SAMPLE_VerifyCmdLength(SAMPLE_AppData.MsgPtr,
                               sizeof(SAMPLE_NoArgs_cmd_t)) == OS_SUCCESS) {
      CFE_EVS_SendEvent(SAMPLE_CMD_DISABLE_INF_EID,
                        CFE_EVS_EventType_INFORMATION,
                        "SAMPLE: Disable command received");
      SAMPLE_Disable();
    }
    break;
  ...
  */
  case SAMPLE_CONFIG_CC:
    if (SAMPLE_VerifyCmdLength(SAMPLE_AppData.MsgPtr,
                               sizeof(SAMPLE_Config_cmd_t)) == OS_SUCCESS) {
      uint32_t config =
          ntohl(((SAMPLE_Config_cmd_t *)SAMPLE_AppData.MsgPtr)
                    ->DeviceCfg); // command is defined as big-endian... need to
                                  // convert to host representation
      CFE_EVS_SendEvent(SAMPLE_CMD_CONFIG_INF_EID,
                        CFE_EVS_EventType_INFORMATION,
                        "SAMPLE: Configuration command received: %u", config);
      /* Command device to send HK */
      status = SAMPLE_CommandDevice(&SAMPLE_AppData.SampleUart,
                                    SAMPLE_DEVICE_CFG_CMD, config);
  ...
  */
  default:
    ...
    SAMPLE_AppData.HkTelemetryPkt.CommandErrorCount++;
    CFE_EVS_SendEvent(
        SAMPLE_CMD_ERR_EID, CFE_EVS_EventType_ERROR,
...
void SAMPLE_ProcessTelemetryRequest(void) {
  ...
  */
  default:
    ...
    SAMPLE_AppData.HkTelemetryPkt.CommandErrorCount++;
    CFE_EVS_SendEvent(
        SAMPLE_DEVICE_TLM_ERR_EID, CFE_EVS_EventType_ERROR,
        "SAMPLE: Invalid command code for packet, MID = 0x%x, cmdCode = 0x%x",
        CFE_SB_MsgIdToValue(MsgId), CommandCode);
    break;
  }
...
void SAMPLE_ReportHousekeeping(void) {
  ...
  if (SAMPLE_AppData.HkTelemetryPkt.DeviceEnabled == SAMPLE_DEVICE_ENABLED) {
    status = SAMPLE_RequestHK(
        &SAMPLE_AppData.SampleUart,
  ...
  if (SAMPLE_AppData.HkTelemetryPkt.DeviceEnabled == SAMPLE_DEVICE_ENABLED) {
    ...
    if (status == OS_SUCCESS) {
      ...
      CFE_EVS_SendEvent(SAMPLE_REQ_DATA_ERR_EID, CFE_EVS_EventType_ERROR,
                        "SAMPLE: Request device data reported error %d",
                        status);
    }
  }
  /* Intentionally do not report errors if disabled */
    ...
    */
    ...
    if (status == OS_SUCCESS) {
      ...
      CFE_EVS_SendEvent(SAMPLE_ENABLE_INF_EID, CFE_EVS_EventType_INFORMATION,
                        "SAMPLE: Device enabled");
    } else {
      SAMPLE_AppData.HkTelemetryPkt.DeviceErrorCount++;
      CFE_EVS_SendEvent(SAMPLE_UART_INIT_ERR_EID, CFE_EVS_EventType_ERROR,
                        "SAMPLE: UART port initialization error %d", status);
    }
  } else {
    SAMPLE_AppData.HkTelemetryPkt.DeviceErrorCount++;
...
int32 SAMPLE_VerifyCmdLength(CFE_MSG_Message_t *msg, uint16 expected_length) {
  ...
  if (expected_length == actual_length) {
    ...
    CFE_EVS_SendEvent(
        SAMPLE_LEN_ERR_EID, CFE_EVS_EventType_ERROR,
