...
namespace Nos3
{
    ...
    const std::string SampleHardwareModel::_sample_stream_name = "sample_stream";

    SampleHardwareModel::SampleHardwareModel(const boost::property_tree::ptree& config) : SimIHardwareModel(config), _stream_counter(0)
    {
        ...
        if (config.get_child_optional("simulator.hardware-model.default-streams")) 
        {
            BOOST_FOREACH(const boost::property_tree::ptree::value_type &v, config.get_child("simulator.hardware-model.default-streams")) // Loop through the default streams for *this* hw model
            {
                ...
                if ((_streaming_data_function_map.find(stream_name) != _streaming_data_function_map.end()) &&
                    ...
                    sim_logger->info("SampleHardwareModel::SampleHardwareModel:  Created default stream name %s starting at %f (start time + %f) with stream period %d milliseconds.", 
                        stream_name.c_str(), _absolute_start_time + initial_stream_time, initial_stream_time, stream_period_ms);
                } else {
                    sim_logger->error("SampleHardwareModel::SampleHardwareModel:  Invalid stream name %s or stream period (must be > 0) %d.", 
        ...
        _time_bus->add_time_tick_callback(std::bind(&SampleHardwareModel::send_streaming_data, this, std::placeholders::_1));
    ...
    void SampleHardwareModel::send_streaming_data(NosEngine::Common::SimTime time)
    {
        const boost::shared_ptr<SampleDataPoint> data_point =
            boost::dynamic_pointer_cast<SampleDataPoint>(_sample_dp->get_data_point());
    ...
    void SampleHardwareModel::create_sample_data(const SampleDataPoint& data_point, std::vector<uint8_t>& out_data)
    {
        ...
        // so that's what this sample is going to do... so scale each of the x, y, z (which are in the range [-1.0, 1.0]) by 32767
